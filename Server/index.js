...
app.get('/',function (req,res) {
  console.log('aaaa');
  res.sendfile('root/oat/WarKu/Client/index.html');
})
/** login **/
...
app.post('/changeEmail',function (req,res) {
  ...
  engine.verify(username,password,function (err,status) {
      if (err) {
        ...
        if (status){
          engine.changeEmail(username,email,function (err,status) {
            if (err) {
              console.log("Server Recieve From Engine : "+err.toString());
              res.end("Something wrong on our server :( Try Again~");
            }
...
app.post('/getPopulation',function (req,res) {
  var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
  console.log("\nCurrent IP : "+ip);
  console.log("Current User : "+online_user[ip]);
  var username = online_user[ip];
  engine.getPopulation(username,function (err,pop) {
    if (err) {
      console.log("Server Recieve From Engine : "+err.toString());
      res.end("Something wrong on our server :( Try Again~");
    }else {
...
app.post('/getStatistic',function (req,res) {
  var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
  console.log("\nCurrent IP : "+ip);
  console.log("Current User : "+online_user[ip]);
  var username = online_user[ip];
  engine.getStatistic(username,function (err,s) {
    if (err) {
      console.log("Server Recieve From Engine : "+err.toString());
      res.end("Something wrong on our server :( Try Again~");
    }else {
...
app.post('/getRanking',function (req,res) {
  ...
  engine.getRanking(rank,function (err,s) {
    if (err) {
      console.log("Server Recieve From Engine : "+err.toString());
      res.end("Something wrong on our server :( Try Again~");
    }else {
