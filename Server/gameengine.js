...
function calculateFinishDate(ah,ami,as) {
  var time = new Date();
  var y = time.getUTCFullYear();
  var m = time.getUTCMonth();
  var d = time.getUTCDate();
  var h = time.getUTCHours();
  var mi = time.getUTCMinutes();
  var s = time.getUTCSeconds();
  var finishDate = new Date(y,m,d,h+resource_info[type].time[level].hour,mi+resource_info[type].time[level].min,s+resource_info[type].time[level].sec);
  return finishDate.getUTCFullYear()+'-'+twoDigits(1+finishDate.getUTCMonth())+'-'+twoDigits(finishDate.getUTCDate())+" "+twoDigits(finishDate.getUTCHours())+":"+twoDigits(finishDate.getUTCMinutes())+":"+twoDigits(finishDate.getUTCSeconds());
}
